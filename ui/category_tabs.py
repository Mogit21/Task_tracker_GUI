...
def render_task_table(main_category_filter: str):
    session = SessionLocal()
    categories = session.query(Category).filter_by(main_category=main_category_filter).all()
    cat_map = {c.id: c.sub_category for c in categories}
    session.close()
    ...
    # Slot-based custom action buttons
    # table.add_slot('body-cell-actions', lambda row: ui.row().classes('gap-2').props('align=center').on('click.stop', None).add(
    ...
    def show_edit_task_dialog(task_id):
        ...
        category_options = {
            c.sub_category: c.id
            for c in session.query(Category).filter_by(main_category=main_category_filter).all()
        }
        session.close()
